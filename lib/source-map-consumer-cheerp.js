...
BasicSourceMapConsumer.prototype._parseMappings =
  function BasicSourceMapConsumer_parseMappings(aStr, aSourceRoot) {
    const mappingsPtr = this._wasm.exports.Mappings.create(aStr);
...
BasicSourceMapConsumer.prototype.eachMapping =
  function BasicSourceMapConsumer_eachMapping(aCallback, aContext, aOrder) {
    ...
    case SourceMapConsumer.GENERATED_ORDER:
      iter = this._mappingsPtr.by_generated_location();
      break;
    case SourceMapConsumer.ORIGINAL_ORDER:
      iter = this._mappingsPtr.by_original_location();
      break;
    default:
    ...
    while( m = iter.next() ) {
      var source = null;
      if (m.get_source() !== NO_DATA) {
        source = this._sources.at(m.get_source());
        source = util.computeSourceURL(this.sourceRoot, source, this._sourceMapURL);
      }

      var name = null;
      if (m.get_name() !== NO_DATA) {
        name = this._names.at(m.get_name());
      }
      var lastColumn = null;
      if (m.get_last_generated_column() !== NO_DATA) {
        lastColumn = m.get_last_generated_column() - 1;
      }

      let mapping = new Mapping();
      mapping.generatedLine = m.get_generated_line();
      mapping.generatedColumn = m.get_generated_column();
      mapping.lastGeneratedColumn = lastColumn;
      mapping.source = source;
      mapping.originalLine = m.get_original_line()!==NO_DATA?m.get_original_line():null;
      mapping.originalColumn = m.get_original_column()!=NO_DATA?m.get_original_column():null;
      mapping.name = name;
...
BasicSourceMapConsumer.prototype.allGeneratedPositionsFor =
  function BasicSourceMapConsumer_allGeneratedPositionsFor(aArgs) {
    ...
    this._getMappingsPtr();
    let all = this._mappingsPtr.all_generated_locations_for(
      source,
      originalLine,
      'column' in aArgs,
      originalColumn
    ...
    while( m = all.next() ) {
      ...
      mappings.push({
        line: m.get_generated_line(),
        column: m.get_generated_column(),
        lastColumn,
      });
    }
    return mappings;
...
BasicSourceMapConsumer.prototype.computeColumnSpans =
  function SourceMapConsumer_computeColumnSpans() {
    ...
    this._getMappingsPtr();
    this._mappingsPtr.compute_column_spans();
...
BasicSourceMapConsumer.prototype.originalPositionFor =
  function SourceMapConsumer_originalPositionFor(aArgs) {
    ...
    var mapping = this._mappingsPtr.original_location_for(needle.generatedLine, needle.generatedColumn, bias);
    if (mapping) {
      if (mapping.get_generated_line() == needle.generatedLine) {
        var source = null;
        if (mapping.get_source() != NO_DATA) {
          source = this._sources.at(mapping.get_source());
          source = util.computeSourceURL(this.sourceRoot, source, this._sourceMapURL);
        }

        var name = null;
        if (mapping.get_name() !== NO_DATA) {
          name = this._names.at(mapping.get_name());
        }

        return {
          source: source,
          line: mapping.get_original_line(),
          column: mapping.get_original_column(),
          name: name
        };
      }
    }
...
BasicSourceMapConsumer.prototype.generatedPositionFor =
  function SourceMapConsumer_generatedPositionFor(aArgs) {
    ...
    var mapping = this._mappingsPtr.generated_location_for(source, needle.originalLine, needle.originalColumn, bias);
    if (mapping) {
      if (mapping.get_source() == needle.source) {
        let lastColumn = null;
        if (mapping.get_last_generated_column() !== NO_DATA)
          lastColumn = mapping.get_last_generated_column() - 1;
        if (this._computedColumnSpans && lastColumn === null) {
          lastColumn = Infinity;
        }
        return {
          line: mapping.get_generated_line(),
          column: mapping.get_generated_column(),
          lastColumn,
        };
      }
    }
