...
BasicSourceMapConsumer.prototype.eachMapping =
  function BasicSourceMapConsumer_eachMapping(aCallback, aContext, aOrder) {
    ...
    while( m = iter.next() ) {
      ...
      mapping.lastGeneratedColumn = m.get_last_generated_column();
      mapping.source = m.get_source();
      mapping.originalLine = m.get_original_line();
      mapping.originalColumn = m.get_original_column();
      mapping.name = m.get_name();
...
BasicSourceMapConsumer.prototype.allGeneratedPositionsFor =
  function BasicSourceMapConsumer_allGeneratedPositionsFor(aArgs) {
    ...
    let m = null;
    while( m = all.next() ) {
      var lastColumn = m.get_last_generated_column();
...
BasicSourceMapConsumer.prototype.generatedPositionFor =
  function SourceMapConsumer_generatedPositionFor(aArgs) {
    ...
    if (mapping) {
      let lastColumn = mapping.get_last_generated_column();
      if (this._computedColumnSpans && lastColumn === null) {
        lastColumn = Infinity;
      }
      return {
