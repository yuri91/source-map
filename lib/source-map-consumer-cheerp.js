...
BasicSourceMapConsumer.prototype.eachMapping =
  function BasicSourceMapConsumer_eachMapping(aCallback, aContext, aOrder) {
    ...
    let iter = this._mappingsPtr.by_generated_location();
    let m = null;
    while( m = iter.next() ) {
      var source = null;
      if (m.has_original_location()) {
        source = this._sources.at(m.source());
        source = util.computeSourceURL(this.sourceRoot, source, this._sourceMapURL);
      }

      var name = null;
      if (m.has_name()) {
        name = this._names.at(m.name());
      }
      var lastColumn = null;
      if (m.has_last_generated_column()) {
        lastColumn = m.last_generated_column() - 1;
      }

      let mapping = new Mapping();
      mapping.generatedLine = m.generated_line();
      mapping.generatedColumn = m.generated_column();
      mapping.lastGeneratedColumn = lastColumn;
      mapping.source = source;
      mapping.originalLine = m.original_line();
      mapping.originalColumn = m.original_column();
      mapping.name = name;
...
BasicSourceMapConsumer.prototype.originalPositionFor =
  function SourceMapConsumer_originalPositionFor(aArgs) {
    ...
    if (mapping) {
      if (mapping.generated_line() === needle.generatedLine) {
        var source = null;
        if (mapping.has_original_location()) {
        ...
        var name = null;
        if (mapping.has_name()) {
          name = this._names.at(mapping.name());
        }
  ...
  function SourceMapConsumer_generatedPositionFor(aArgs) {
    ...
    if (mapping) {
      if (mapping.source()=== needle.source) {
        let lastColumn = null;
        if (mapping.has_last_generated_column())
          lastColumn = mapping.last_generated_column() - 1;
        if (this._computedColumnSpans && lastColumn === null) {
          lastColumn = Infinity;
        }
        return {
